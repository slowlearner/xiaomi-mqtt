...
log.info(`checking for internet before starting up`);
isOnline().then(function (result) {
  ...
  log.info(`connection established, starting up`);
  var params = {
    "config": config,
  ...
  log.warn(`could not establish remote internet connection ${e.message}`);
  process.exit(2);
})
